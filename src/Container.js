...
class Container extends Component {
  ...
  clubButtonClickHandler(clubId, parentClub) {
    ...
    const selectedClubs =
      this.state.selectedNodes[0] === parentClub
        ? [parentClub, club]
        : [club, parentClub];
  ...
  createClubTable() {
    ...
    let tableRows = headers
      ...
      .concat(this.createTable("Most Booked Artists", this.mostCommonArtists()))
      .concat(this.createTable("Most Similar Clubs", this.mostSimilarClubs()));
    ...
    const header =
      rows.length !== 2 ? null : (
        <thead>
          <tr>
            <th key={"empty"} />
            {this.state.selectedNodes.map((e) => (
              <th key={e.id}>{e.id}</th>
            ))}
          </tr>
        </thead>
  ...
  updateWindowDimensions = () => {
    ...
    this.setState({
      ...
      height: h,
      svgWidth: w,
      svgHeight: h - this.controlHeight,
    });
  };
