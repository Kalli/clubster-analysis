...
class ClusterChart extends Chart {
  ...
  createGraph(nodes) {
    ...
    this.simulation = forceSimulation()
      ...
      .force(
        "collide",
        forceCollide((d) => d.radius + padding)
      );
  ...
  decay = () => {
    ...
    const t = timer((elapsed) => {
      ...
      this.simulation.force("collide").strength(Math.pow(dt, 2));
      if (dt >= 1.0) {
        t.stop();
  ...
  drawGraph = (nodes, clickHandler, selectedNodes) => {
    ...
    this.node.exit().transition().duration(2000).style("opacity", 0).remove();
    ...
    this.simulation.on("tick", this.tick).nodes(nodes);
    this.initial = false;
  };
